...
public partial class App_NewUI_EnergyModelingEquipment : System.Web.UI.Page
{
    Int32[] indexes;
    public ArrayList arrayList = new ArrayList();
    public Hashtable ht_zone_equipment = new Hashtable();
        ...
        try
        {
            if (SessionController.Users_.UserId != null)
            {
                if (!IsPostBack)
                {
                    ...
                    ViewState["SelectedEquipmentId"] = null;
                    ViewState["zone_equipment"] = null;
                    GetEnergyModelAddedEquipments();
                    BindEquipments();
                }
            }
    ...
    private void GetEnergyModelAddedEquipments()
    {
        ...
        try
        {
            if (SessionController.Users_.Em_facility_id != null && SessionController.Users_.Profileid != null)
            {
                ...
                ds = obj_energy_plus_client.Get_Energy_Modeling_Project_Zone_Equipment(obj_energy_plus_model, SessionController.ConnectionString);
    ...
    public void ConvertDataSetToArrayList(DataSet ds)
    {
        try
        {
            ...
            foreach (DataRow dtRow in ds.Tables[0].Rows)
            {
                ...
                string equipment_id = dtRow["equipment_id"].ToString();

                if (!arrayList.Contains(equipment_id))
                {
                    arrayList.Add(equipment_id);
                }
                if (!ht_zone_equipment.ContainsKey(equipment_id))
                {
                    ht_zone_equipment.Add(equipment_id, zone_id);
                }
            }
            ViewState["SelectedEquipmentId"] = arrayList;
            ViewState["zone_equipment"] = ht_zone_equipment;
    ...
    protected void rg_equipments_ItemCommand(object source, GridCommandEventArgs e)
    {
        try
        {
            if (hdn_row_index.Value != "")
            {
                ...
                if (e.CommandName == "attributes")
                {
                    ...
                    if (rg_equipments.MasterTableView.Items[e.Item.ItemIndex].Expanded)
                    {
                        if (hf_mode.Value == "edit")
                        {
                            hfExpand.Value = "expanded";
                            rg_equipments.MasterTableView.Items[e.Item.ItemIndex].Expanded = true;
                            create_attributes_for_space(e);
                            foreach (GridDataItem item in rg_equipments.Items)
                            {
                                if (item.Selected == true)
                                {
                                    System.Web.UI.HtmlControls.HtmlTable tbl_img = (System.Web.UI.HtmlControls.HtmlTable)rg_equipments.MasterTableView.Items[item.ItemIndex].FindControl("tbl_edit");
                        ...
                        {
                            hfExpand.Value = "no";
                            rg_equipments.MasterTableView.Items[e.Item.ItemIndex].Expanded = false;
                        }
                        if (hdn_row_deselected.Value == "row_deselected")
                        {
                            hfExpand.Value = "no";
                            rg_equipments.MasterTableView.Items[e.Item.ItemIndex].Selected = false;
                            System.Web.UI.HtmlControls.HtmlTable tbl_img = (System.Web.UI.HtmlControls.HtmlTable)rg_equipments.MasterTableView.Items[e.Item.ItemIndex].FindControl("tbl_edit");
                            tbl_img.Style.Add("display", "none");
                        }
                        else
                        {
                ...
                if (e.CommandName == "save")
                {
                    ...
                    //  rg_equipments.MasterTableView.Items[(e.Item.ItemIndex) + 1].Expanded = true;
                    edit_attributes_for_space(e);
                    foreach (GridDataItem item in rg_equipments.Items)
                    {
                        if (item.Selected == true)
            ...
            else
            {
                if ( e.Item!=null && e.Item.ItemIndex >= 0)
                {
                    ...
                    rg_equipments.MasterTableView.Items[e.Item.ItemIndex].Selected = false;
                    hfExpand.Value = "no";
                    Hfindexes.Value = Convert.ToString(e.Item.ItemIndex);
                }
            }
        }
        catch (Exception ex)
    ...
    protected void rg_equipments_ColumnCreated(object sender, GridColumnCreatedEventArgs e)
    {
    ...
    protected void rg_equipments_PreRender1(object sender, EventArgs e)
    {
        foreach (GridNestedViewItem item in rg_equipments.MasterTableView.GetItems(GridItemType.NestedView)) // loop through the nested items of a NestedView Template
        {
            ...
            rg_equipments.Items[Convert.ToInt32(Hfindexes.Value)].Selected = false;
        ...
        //foreach (GridItem item in rg_equipments.Items)
        //{
        //    if ()
    ...
    protected void rg_equipments_PageSizeChanged(object sender, GridPageSizeChangedEventArgs e)
    {
        try
        {
            GetSelectedRows();
    ...
    protected void rg_equipments_PageIndexChanged(object sender, GridPageChangedEventArgs e)
    {
        try
        {
            GetSelectedRows();
    ...
    protected void rg_equipments_OnSortCommand(object sender, GridCommandEventArgs e)
    {
        BindEquipments();
    }
    protected void rg_equipments_ItemEvent(object sender, GridItemEventArgs e)
    {
    ...
    protected void rg_equipments_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            GridDataItem dataItem = (GridDataItem)e.Item;
            string id = Convert.ToString(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["pk_energy_modeling_project_equipments_linkup"]);
            string pk_asset_id = Convert.ToString(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["pk_asset_id"]);
            ...
            if (!string.IsNullOrEmpty(id))
            {
                ...
                if (!string.IsNullOrEmpty(hf_asset_ids.Value))
                {
                    if ((hf_asset_ids.Value).IndexOf(pk_asset_id) == -1)
                        hf_asset_ids.Value = hf_asset_ids.Value + ',' + pk_asset_id;
                }
                else
                {
                    hf_asset_ids.Value = pk_asset_id;
                }
            }
    ...
    protected void rg_equipments_ItemCreated1(object sender, GridItemEventArgs e)
    {
      
    }
    protected void btn_addAssetstoProject_Click(object sender, EventArgs e)
    {

        AddEnergyModelingEquipment();
    }
    #endregion

    #region private methods
    protected void BindEquipments()
    {
        ...
        try
        {
            ...
            mdl_ep.Asset_id = Guid.Empty;

            ds = ctrl_ep.proc_get_energy_modeling_equipment(mdl_ep, SessionController.ConnectionString);
            rg_equipments.DataSource = ds;
            rg_equipments.DataBind();
    ...
    protected void AddEnergyModelingEquipment()
    {
        EnergyPlusClient ctrl_ep = new EnergyPlusClient();
        EnergyPlusModel mdl_ep = new EnergyPlusModel();
        DataSet ds = new DataSet();
        ...
        GetSelectedRows();
        if (ViewState["zone_equipment"] != null)
        {
            Hashtable ht_zone_equip = (Hashtable)ViewState["zone_equipment"];
            if (ht_zone_equip != null)
            {
                foreach (string k in ht_zone_equip.Keys)
                {
                    asset_ids = asset_ids + k + ",";
                    Space_ids = Space_ids + ht_zone_equip[k] + ",";
                }
            }
        }

        asset_ids = asset_ids.TrimEnd(',');
        Space_ids = Space_ids.TrimEnd(',');

        Space_ids = Convert.ToString(hf_space_ids.Value);
        asset_ids = Convert.ToString(hf_asset_ids.Value);
        class_ids = Convert.ToString(hfEquipment_class.Value);
        zone_ids = Convert.ToString(hf_zone_id.Value);
        mdl_ep.spaceids = Space_ids;
        mdl_ep.assetids = asset_ids;
        mdl_ep.pk_profileid = new Guid(SessionController.Users_.Profileid);
        mdl_ep.Fk_facility_id = new Guid(SessionController.Users_.Em_facility_id);
        mdl_ep.User_id = new Guid( SessionController.Users_.UserId);
        mdl_ep.classids = class_ids;
        mdl_ep.zoneids = zone_ids;
        ctrl_ep.proc_insert_update_EnergyModeling_projects_equipments(mdl_ep, SessionController.ConnectionString);
    ...
    void saveattributes(object sender, EventArgs e)
    {
        ...
        DataSet ds = EnergyPlusClient.proc_update_energy_modeling_equipment_attributes(EnergyPlusModel, SessionController.ConnectionString);
    ...
    void create_attributes_for_space(GridCommandEventArgs e)
    {
        ...
        Guid pk_equipment_id = new Guid(Convert.ToString(item.GetDataKeyValue("pk_asset_id")));
        Guid group_id;
        if ((!(string.IsNullOrEmpty(Convert.ToString(item.GetDataKeyValue("group_id"))))))
            group_id = new Guid(Convert.ToString(item.GetDataKeyValue("group_id")));
        else
            group_id = Guid.Empty;
        hf_equipment_id.Value = Convert.ToString(pk_equipment_id);
        ...
        Table tblSpace_attributes = (Table)pnlAttributes.FindControl("tblequipment_attributes");
        ...
        EnergyPlusClient EnergyPlusClient = new EnergyPlusClient();
        EnergyPlusModel EnergyPlusModel = new EnergyPlusModel();
        EnergyPlusModel.Asset_id = pk_equipment_id;
        EnergyPlusModel.Modeling_Obj_Id = group_id;
        ds = EnergyPlusClient.proc_get_energy_modeling_equipment_attributes(EnergyPlusModel, SessionController.ConnectionString);
        EnergyPlusModel.spaceid = pk_equipment_id;
      
        dsObjects = EnergyPlusClient.proc_get_energymodeling_objects_for_equipment(EnergyPlusModel, SessionController.ConnectionString);

        rcbModelingObjects.DataSource = dsObjects;
        rcbModelingObjects.DataTextField = "group_name";
        rcbModelingObjects.DataValueField = "pk_group_name_id";
    ...
    void edit_attributes_for_space(GridCommandEventArgs e)
    {
        ...
        Table tbl_edit_space_attributes = (Table)pnl_edit_attributes_table.FindControl("tbl_edit_equipment_attributes");
        ...
        try
        {
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                TestID = Guid.Parse(ds.Tables[0].Rows[row]["pk_asset_attribute_value_id"].ToString());
    ...
    protected void BindZones()
    {
        ...
        rg_equipments.DataSource = ds.Tables[0];
        rg_equipments.DataBind();
    ...
    protected void GetSelectedRows()
    {
        try
        {
            ...
            foreach (GridDataItem item in rg_equipments.Items)
            {
                string strIndex = rg_equipments.MasterTableView.CurrentPageIndex.ToString();
                string equipment_id = item["pk_asset_id"].Text;
                string zone_id = item["space_id"].Text;

                if (item.Selected)
                {
                    if (!arrayList.Contains(equipment_id.ToString()))
                    {
                        arrayList.Add(equipment_id.ToString());
                    }
                    if (!ht_zone_equipment.ContainsKey(equipment_id))
                    {
                        ht_zone_equipment.Add(equipment_id, zone_id);
                ...
                else
                {
                    if (arrayList.Contains(equipment_id.ToString()))
                    {
                        arrayList.Remove(equipment_id.ToString());
                    }
                    if (ht_zone_equipment.ContainsKey(equipment_id))
                    {
                        ht_zone_equipment.Remove(equipment_id);
                    }
                }
            }
    ...
    protected void ReSelectedRows()
    {
        try
        {
            foreach (GridDataItem item in rg_equipments.Items)
            {
                string pk_asset_id = item["pk_asset_id"].Text;
                if (arrayList.Contains(pk_asset_id.ToString()))
                {
                    item.Selected = true;
                }
            }
    ...
    protected void SessionToArrayList()
    {
        try
        {
            if (ViewState["SelectedEquipmentId"] != null)
            {
                arrayList = (ArrayList)ViewState["SelectedEquipmentId"];
            }
            if (ViewState["zone_equipment"] != null)
            {
                ht_zone_equipment = (Hashtable)ViewState["zone_equipment"];
            }
        }
        catch (Exception ex)
        {
    ...
    protected void rg_equipments_DataBound(object sender, EventArgs e)
    {
        try
        {
            ReSelectedRows();
