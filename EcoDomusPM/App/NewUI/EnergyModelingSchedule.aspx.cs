...
public partial class App_NewUI_EnergyModelingSchedule : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (SessionController.Users_.UserId != null)
        {
            if (!Page.IsPostBack)
            {
                if (SessionController.Users_.UserId != null)
                {
                    BindScheduleTypeDropDown();
                    if (cmb_schedule_type.Items.Count > 0)
                    {
                        cmb_schedule_type.SelectedItem.Value= "Schedule:Compact";
                    }

                    BindEnergyModelingScheduleGrid();
                    rg_schedule.MasterTableView.GroupsDefaultExpanded = false;
                    if (this.rg_schedule.MasterTableView.GroupByExpressions.Count > 0)
                    {
                        //refresh on RreRender if grid not rebound
                        this.rg_schedule.MasterTableView.SetLevelRequiresBinding();
                    }
    ...
    private void BindScheduleTypeDropDown()
    {
        ...
        try
        {
            ...
            cmb_schedule_type.DataSource = ds;
            cmb_schedule_type.DataTextField = "object_name";
            cmb_schedule_type.DataValueField = "object_name";
            cmb_schedule_type.DataBind();
            if (!hdnscheduletypeid.Value.Equals(""))
            {
                cmb_schedule_type.SelectedValue = hdnscheduletypeid.Value;
            }
        }
        catch (Exception ex)
        {
    ...
    private void BindEnergyModelingScheduleGrid()
    {
        EnergyPlusClient obj_energy_plus_client = new EnergyPlusClient();
        EnergyPlusModel obj_energy_plus_model = new EnergyPlusModel();
        DataSet ds = new DataSet();
        try
        {
            if (SessionController.Users_.Em_facility_id != null)
            {
                ...
                if (cmb_schedule_type.Items.Count > 0)
                {
                    obj_energy_plus_model.Object_name = cmb_schedule_type.SelectedItem.Value;
                ...
                {
                    rg_schedule.DataSource = string.Empty;
                    rg_schedule.DataBind();
                }
            }
    ...
    protected void rg_schedule_ItemCommand(object sender, GridCommandEventArgs e)
    {
        EnergyPlusClient obj_energy_plus_client = new EnergyPlusClient();
        ...
        try
        {
            ...
            rg_schedule.MasterTableView.GroupsDefaultExpanded = true;
            if (this.rg_schedule.MasterTableView.GroupByExpressions.Count > 0)
            {
                //refresh on RreRender if grid not rebound
                this.rg_schedule.MasterTableView.SetLevelRequiresBinding();
    ...
    protected void rg_schedule_PageIndexChanged(object sender, GridPageChangedEventArgs e)
    {
        try
        {
            GetSelectedRows();
    ...
    protected void rg_schedule_PageSizeChanged(object sender, GridPageSizeChangedEventArgs e)
    {
        try
        {
            GetSelectedRows();
    ...
    protected void rg_schedule_SortCommand(object sender, GridSortCommandEventArgs e)
    {
        try
        {
            //GetSelectedRows();
    ...
    protected void rg_schedule_ItemDataBound(object sender, GridItemEventArgs e)
    {
        EnergyPlusClient obj_energy_plus_client = new EnergyPlusClient();
        EnergyPlusModel obj_energy_plus_model = new EnergyPlusModel();
        if (e.Item is GridGroupHeaderItem)
    ...
    protected void rg_schedule_ItemCreated(object sender, GridItemEventArgs e)
    {
    ...
    protected void GetSelectedRows()
    {
        try
        {
            ...
            foreach (GridGroupHeaderItem item in rg_schedule.MasterTableView.GetItems(GridItemType.GroupHeader))
            {
    ...
    protected void rg_schedule_DataBound(object sender, EventArgs e)
    {
        try
        {
            ReSelectedRows();
