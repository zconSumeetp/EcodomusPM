...
public partial class App_NewUI_EnergyModelingRunSimulation : System.Web.UI.Page
{
    ...
    protected void ibtn_start_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            string pk_file_id = "";
            //string fName = null;
            //ProgressBarSimulation(20, "Simulation is in progress.");

            //ProgressBarSimulation(40, "Simulation is in progress.");

            //ProgressBarSimulation(60, "Simulation is in progress.");
            ...
            if (SessionController.Users_.Profileid == null)
            {
                // IDF_Exporter.pk_project_id = Guid.Empty.ToString(); //"00000000-0000-0000-0000-000000000000";
            }
            else
            {
                //  IDF_Exporter.pk_project_id = SessionController.Users_.Profileid;
            }
            EnergyPlusModel obj_energy_plus_model = new EnergyPlusModel();
            EnergyPlusClient obj_energy_plus_client = new EnergyPlusClient();
            //IDF_Exporter.ecodomus_client.sql_connection.ConnectionString = obj_energy_plus_client.Get_Connection_String(obj_energy_plus_model, SessionController.ConnectionString);
            ...
            if (file_name != null)
            {
                ...
                EnergyModelingtIDFExport IDF_Exporter = new EnergyModelingtIDFExport();

                //IDF_Exporter.IDF_Export(newfilePath, SessionController.Users_.Em_facility_id);
                // close the db

                //ProgressBarSimulation(80, "Simulation is in progress.");


                //Running Batch File
                string path = Path.Combine(Server.MapPath(WebConfigurationManager.AppSettings["EnergyPlusFilePath"] + SessionController.Users_.Em_facility_id), file_name);
                InsertUpdateEnergyModelingSimulationRequest(file_name, path, pk_file_id);
                ImportSqliteFile importsqlitedata = new ImportSqliteFile();
                // importsqlitedata.GetRunSimulationRequest(Server.MapPath(WebConfigurationManager.AppSettings["EnergyPlusFilePath"] + SessionController.Users_.Em_facility_id),file_name,"","");
                ProgressBarSimulation(10, "Simulation started ...");
               
                //ProgressBarSimulation(0, "Simulation Completed.");
            }
            //}
        }
        catch (Exception ex)
