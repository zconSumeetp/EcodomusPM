...
public partial class App_LoginPM : System.Web.UI.Page
{
    ...
    protected override void InitializeCulture()
    {
        ...
        {
            if (!TryGetCultureFromCookie(out culture))
            {
                culture = "en-US";
            }
    ...
    private bool TryGetCultureFromCookie(out string culture)
    {
        if (Request.Cookies["SelectedCulture"] != null)
        {
            culture = HttpUtility.UrlDecode(Request.Cookies["SelectedCulture"].Value);
            return true;
        }
        culture = null;
        return false;
    ...
    protected void Bindlangauage()
    {
        ...
        try
        {
            ...
            if (Session["Culture"] != null)
            {
                ...
                var item = rcm_languages.Items.FindItem(x => x.Attributes["culture"] == culture);
                if (item != null)
                {
                    item.Selected = true;
    ...
    protected void ChkLogin(string username, string password)
    {
        try
        {
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                //if (Convert.ToString(login_chk) != "" || Request.Url.Host == "localhost")
                if (true)
                {
                    /*----Added for Licensing ---*/
                    ...
                    if (isValidUser)
                    {
                        LoginClient loginobject = new LoginClient();
                        LoginModel loginmodel = new LoginModel();
                        loginmodel.Application_flag = "PM";
                        loginmodel.UserId = new Guid(SessionController.Users_.UserId.ToString());

                        loginmodel.session_type = "IN";
                        loginmodel.IpAddress = Request.ServerVariables["REMOTE_ADDR"].ToString();
                        loginmodel.LoginId = new Guid(SessionController.Users_.UserLoginDetailId.ToString());

                        loginmodel.SessionInTime = DateTime.Now.ToUniversalTime(); //For ActivityLogReport: by kongkun
                        
                        loginobject.InsertLoginUsersessionDetail(loginmodel);
    ...
    protected void rcm_languages_OnItemDataBound(object sender, RadMenuEventArgs e)
    {
        var dataRowView = (DataRowView)e.Item.DataItem;
        var culture = (string) dataRowView["culture"];

        e.Item.Attributes["culture"] = culture;
    }
}