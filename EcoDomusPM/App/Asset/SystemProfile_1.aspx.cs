...
public partial class App_Asset_SystemProfile_1 : System.Web.UI.Page
{
    string Master_flag = "";
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (hf_uniclass.Value != "")
                {
                    if (hf_uniclass.Value == "Y")
                    {
                        ...
                        hf_lblOmniClassid.Value = ds.Tables[0].Rows[0]["uniclassid"].ToString();
                    }
                    else
                    {
    ...
    protected void getparentsystemname()
    {
        ...
        objSystemsModel.SystemId = new Guid(hfSystemId.Value);
        DataSet ds = new DataSet();
        ds = objSystemsClient.GetParentSystem(objSystemsModel, SessionController.ConnectionString);
        if (ds.Tables[0].Rows.Count > 0)
            lblparentsystemname.Text = ds.Tables[0].Rows[0][0].ToString();
